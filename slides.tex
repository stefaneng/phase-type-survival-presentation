...
\subtitle{with Applications to Survival Analysis}
\author{Stefan Eng\\
    Advisor: Jeff Steif
}
\institute{University of Gothenburg/Göteborgs Universitet, Sweden}
\date{July 21, 2020}
...
\begin{itemize}
    \item Algebraic computations of phase-type distributions for the contact process and voter model on complete graph with $n = 2, 3, 4$ nodes as well as numeric computations of other graph configurations.
    \item Limiting behavior for the absorption time of the contact process on a finite complete graph with $n$ nodes.
    \item Algebraic computation of the expected value of the absorption time of voter model on a finite complete graph with $n$ nodes.
    \item R package containing simulation code for interacting particle systems with different graph configurations as well as all visualizations.
\end{itemize}
\end{frame}
...
\begin{frame}{Limiting Distribution}
    \begin{defn}[Approaching in Distribution]
    If $X_n$ and $X$ are random variables with cumulative distribution functions $F_n$ and $F$ then $X_n$ approaches $X$ \textit{in distribution}, notated $X_n \Rightarrow X$, if
    $$
    ...
    \begin{example}[Central Limit Theorem]
        Let $X_1, \ldots, X_n$ be i.i.d random variables with $E[X_i] = \mu$ and finite variance $\Var(X_i) = \sigma^2$. Denote ${\bar {X}}_{n}$ as the sample mean. Then
        $$
        {\frac {{\bar {X}}_{n}-\mu }{\sigma /\sqrt{n}}} \Rightarrow N(0,1)
        $$
        as $n \to \infty$
    \end{example}
    % TODO: Add example of limiting distribution
\end{frame}
...
\begin{frame}{Time homogeneous Markov Chain}
We say that a discrete-time Markov chain $(X_n)$ with state space $S$ is \textbf{time homogeneous} if
    ...
    All Markov chains in this presentation are assumed to be time homogeneous.
\end{frame}

\begin{frame}{Graphical Representation of Discrete Markov Chain}
    \begin{figure}[H]
    \centering
   ...
   \node[node style] at (0, 0)     (2)     {Healthy};
   \node[node style] at (6, 0)     (1)     {Diseased};
   \node[node style] at (3, -4) (0) {Death};

   \draw (2) edge[bend left] node[yshift=3mm]{$p_1$} (1);
   \draw (1) edge[bend left] node[yshift=-3mm]{$p_2$}(2);
   \draw (2) edge[left] node[xshift=-3mm]{$1 - p_1$} (0);
   %
   \draw (1) edge[left] node[xshift=15mm]{$1 - p_2$} (0);
   \end{tikzpicture}
\end{figure}   
\end{frame}

\begin{frame}{Continuous-time Markov Chain}
The process $(X(t))$ is a \textbf{continuous-time Markov chain} (CTMC) with index set $[0, \infty)$ and finite or countable state space $S$ if
$$
...
\begin{frame}{Graphical Representation of CTMC}
A graphical representation of a continuous-time Markov chain representing the illness-death model. The rates are $\lambda_1, \lambda_2, \lambda_3$.
    \begin{figure}[H]
    \centering
   \begin{tikzpicture}[start chain = going right,
   -Triangle, every loop/.append style = {-Triangle}]
   ...
   \node[node style] at (0, 0)     (2)     {Healthy};
   \node[node style] at (6, 0)     (1)     {Diseased};
   \node[node style] at (3, -4) (0) {Dead};

   \draw (2) edge[left] node[yshift=3mm]{$\lambda_1$} (1);
   \draw (2) edge[left] node[xshift=-3mm]{$\lambda_2$} (0);
   %
   \draw (1) edge[left] node[xshift=15mm]{$\lambda_3$} (0);
   \end{tikzpicture}
   \caption{Continuous-time Markov chain for illness-death model}
    \end{figure}   
\end{frame}
...
$$
and the probability density function of $\tau$ is
$$
...
\begin{frame}{Example - Coxian Distribution}
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{figures/coxian_distribution.png}
    \caption{Coxian distribution with 5 transient states. The labels on the edges are the transition rates.}
\end{figure}
\end{frame}

\begin{frame}{Example - Coxian Distribution}
A Coxian distribution has the following parameters for a phase-type distribution:
An initial distribution $\boldsymbol{\alpha} = (1, 0, 0, \ldots, 0)$ and
    $$
    ...
    $$
with
$\mathbf{S}_0 = ((1 - p_1)\lambda_1, (1 - p_2)\lambda_2, \ldots, (1 - p_{k - 1}) \lambda_{k - 1}, \lambda_k)^T$
\end{frame}
...
\begin{itemize}
    ...
    \item Infection rate $\lambda$, nodes with 1 are called "infected".
    ...
    \item Denote the extinction/absorption time as $\tau_{G}$ (which always occurs on finite graphs).
    \item If $S$ is finite, the transition rates can be described as follows
    \begin{align*}
    1 &\to 0 \text{ at rate } 1\\
    0 &\to 1 \text{ at rate }  \lambda K
    \end{align*}
...
\begin{frame}{Contact process 2 nodes projection}
Project the Markov chain to the number of 1's on the graph.
There are 3 states for this process.

\begin{figure}
    \centering
    \includegraphics[width=.9\textwidth]{figures/contact_process_projection.png}
\end{figure}
\end{frame}

\begin{frame}{Contact process 2 nodes projection}
This new projected continuous-time Markov chain has the following graphical representation.
\begin{figure}
    \centering
   \begin{tikzpicture}[start chain = going right,
   -Triangle, every loop/.append style = {-Triangle}]
...
\end{tikzpicture}
\caption{Projection states for the 2 node contact process}
\end{figure}
\end{frame}
...
\begin{frame}{Contact process 2 node survival function}
\begin{figure}[H]
  \centering
    \includegraphics[width=.80\textwidth]{figures/complete_2_contact_survival.png}
   \caption{Survival functions for $\tau_{K_2}$ for $\lambda = 0, 1, 2, 3, 4$}
\end{figure}
\end{frame}


\begin{frame}{Contact process 2 node phase-type hazard function}
The hazard function converges to the smallest non-zero absolute value of the eigenvalues which is 0.58 for the 2 nodes complete contact process. 
\begin{figure}[H]
  \centering
    \includegraphics[width=.80\textwidth]{figures/complete_2_contact_hazard_pi.png}
   \caption{Hazard functions for $\tau_{K_2}$ when starting at state 2 and state 1.}
  \label{fig:contact_2_phase_densities}
\end{figure}
\end{frame}
...
% \begin{frame}{Contact process 3 node survival function}
% \begin{figure}[H]
%   \centering
%     \includegraphics[width=.80\textwidth]{figures/complete_3_contact_survival.png}
%   \caption{Survival functions for $\tau_{K_3}$ for $\lambda = 0, 1, 2, 3, 4$ on the complete three node contact process}
% \end{figure}
% \end{frame}

% \begin{frame}{Contact process 3 node phase-type hazard function}
% The hazard function converges to the smallest non-zero absolute value of the eigenvalues which is 0.66 for the 3 nodes complete contact process.
% \begin{figure}[H]
%   \centering
%     \includegraphics[width=.80\textwidth]{figures/complete_3_contact_hazard_pi.png}
%       \caption{Hazard functions for $\tau_{K_3}$ when starting at states 3, 2 and 1.}
% \end{figure}
% \end{frame}

\begin{frame}{Contact process $n$ nodes}
    \begin{figure}[H]
    \centering
   \begin{tikzpicture}[
...
Now let $N_1, N_2, \ldots, N_n$ be the geometric number of visits to states $1, 2, \ldots, n$ respectively.
...
\begin{frame}{Voter Model}
    The Voter Model is another interacting particle system which can be thought of as people having opinions on a subject (such as an election).

    Each person waits an exponentially distributed time with rate 1 and selects a random neighbor to conform to their opinion.
    
    The dynamics of the system are not changed by interchanging 1 and 0 so we still have a single absorbing state when the system is either all 1's or all 0's.
\end{frame}
% \begin{frame}{Voter Model}
% \begin{itemize}
...
%     Equivalently, each person waits an exponential time proportional to the number of people in disagreement and takes their opinion.
%     \begin{figure}[H]
%   \centering
...
\begin{frame}{Voter Model Results}
\begin{itemize}
    \item Phase-type distributions were computed exactly for the complete graph with $n = 2, 3, 4$.
    \item Created simulations for 1 and 2 dimensional voter models on other graph configurations.
    \item In the general case for finite $n$, a closed form for the expected value of the phase-type distribution.
\end{itemize}
\end{frame}
...
\section{Biostatistics Connection}

\begin{frame}{Survival Analysis}
\begin{itemize}
    \item \cite{aalen1995} describes phase type distributions with a focus on hazard rates.
    \item Multi-state models are time-to-event models where the response is a state from a Markov chain.
    \item Generally there is one absorbing state so that we can use phase-type distribution theory.
\end{itemize}
\end{frame}

\begin{frame}{Multiple models for survival data}
    \begin{figure}
        \centering
        \includegraphics[width=.9\textwidth]{figures/multi-state_models.png}
        \caption{Various Markov Chains for time-to-event data.}
    \end{figure}
\end{frame}

\begin{frame}{Multi-state Models}
    \begin{itemize}
        \item Hazard rates are generalized to \textit{transition intensities} which is the instantaneous risk of transitioning between states $i \to j$. Let $T$ denote the time of transition from state $i$ to state $j$.
    $$
    \lambda_{ij}(t) = \lim_{dt \downarrow 0} \frac{P(T < t + dt | T \geq t)}{dt}
    $$
    \item A full overview of competing risks and multi-state models is available from \cite{putter2007}.
    \end{itemize}
\end{frame}

\begin{frame}{Multi-state Model Fitting}
    \begin{itemize}
    \item \cite{msm} developed the \textit{msm} R package to fit multi-state models.
    \begin{itemize}
        \item Fits both time-homogeneous and inhomogeneous Markov models and well as hidden Markov models with individual or time-dependent covariates.
    \end{itemize}
    \item \cite{mstate} developed the \textit{mstate} R package for competing risks and multi-state models.
    \begin{itemize}
        \item Allows for non-Markov models such as semi-Markov and extended semi-Markov models (hazard from $i \to j$ can depend on time of arrival at state $i$)
    \end{itemize}
\end{itemize}
\end{frame}
