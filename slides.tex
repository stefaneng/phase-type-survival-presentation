...
\begin{frame}{Limiting Distribution}
    ...
    \begin{example}[Central Limit Theorem]
        ...
        $$
        {\frac {{\bar {X}}_{n}-\mu }{\sigma /\sqrt{n}}} \Rightarrow N(0, \sigma^2)
        $$
        as $n \to \infty$
    \end{example}
...
A graphical representation of a continuous-time Markov chain representing the illness-death model.
    \begin{figure}[H]
    \centering
   \begin{tikzpicture}[start chain = going right,
   -Triangle, every loop/.append style = {-Triangle}]
...
\begin{figure}
    ...
    \caption{Coxian Distribution with 5 transient states}
\end{figure}
\end{frame}
...
The hazard function converges to the smallest absolute value of the non-zero eigenvalues of the matrix $S$. In this case: 0.58
\begin{figure}[H]
  \centering
...
\begin{itemize}
    \item Phase-type distributions were computed exactly complete graph for $n = 2, 3, 4$
...
\begin{itemize}
    \item Odd Aalen connected the theory of phase-type distributions with a focus on hazard rates in \cite{aalen1995}.
    \item 
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Standard survival analysis assumes that the response is dichotomous.
    \item Multi-state models replace 
    \item 
\end{itemize}
