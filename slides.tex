...
\institute{University of Gothenburg/Göteborgs Universitet}
\date{July 21, 2020}
...
\begin{frame}{Limiting Distribution}
    \begin{defn}
    If $X_n$ and $X$ are random variables with cumulative distribution functions $F_n$ and $F$ then $X_n$ approaches in distribution to $X$, notated $X_n \Rightarrow X$, if
    $$
    ...
    \begin{example}[Central Limit Theorem]
        Let $X_1, \ldots, X_n$ be i.i.d random variables with $E[X_i] = \mu$ and finite variance $\Var(X_i) = \sigma^2$. Then
        $$
...
\begin{frame}{Multi-state Models}
    \begin{itemize}
        \item Hazard rates are generalized to \textit{transition intensities} which is the instantaneous risk of transitioning between states $i \to j$
    $$
